...
[rho,p] = corr(A)  % correlation coefficient & associated p-values
 ...
 if i > counter
  for j = 2:n      % Loop for columns
   if j > counter+1
    ...
    P = polyfit(X,Y,1);      % 1st order polynomial (linear)
    Yfit = P(1)*X+P(2);      % y-values of the fitted line
    hold;                    % Allow superimposed plots
    plot(X,Yfit,'r-');       % Fitted line (color: red, type: solid)
    hold;
    
    % Define the ranges for the x- and y-axis %
    axis([min(X)-.1*range(X) max(X)+.1*range(X) min(Y)-.1*range(Y)...
     max(Y)+.25*range(Y)]);
    xLimit = get(gca, 'xlim');
    ...
    if loc == 1
     text(xLoc+1.2*range(X),yLoc,regexprep(caption, '_', '-'),...
