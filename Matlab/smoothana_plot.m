...
% -------------------------------------------------------------------------
% savefile
savefile_stats = ['output_smoothana' '/' 'smoothana_' plot_type '_nsets_' num2str(n_sets) ...
    '_smoothtype_' smoothtype '_wsize_' num2str(min(wsize)) num2str(mean(diff(wsize))) num2str(max(wsize)) ...
    '_nperm_' num2str(n_perm) '_online_' num2str(online) '.mat'];
savefile_fig = ['figures_smoothana' '/' 'smoothana_' plot_type '_nsets_' num2str(n_sets) ...
    '_smoothtype_' smoothtype '_wsize_' num2str(min(wsize)) num2str(mean(diff(wsize))) num2str(max(wsize)) ...
    '_nperm_' num2str(n_perm) '_online_' num2str(online) '.svg'];
    ...
    for s = 1:2
        for w = 1:length(wsize)
            if strcmp(plot_type, 'corr') || strcmp(plot_type, 'z')
                input = load(['correlations/smoothing' '/' 'corr' '_story_' num2str(s) '_nsets_' num2str(n_sets) ...
                    '_smoothtype_' smoothtype '_wsize_' num2str(wsize(w)) ...
                    '_dosplit_' num2str(0) '_splitby_' '' '_split_' num2str(0) ...
                    '_splittrun_' num2str(0) '_trunc_no' '_nperm_' num2str(n_perm) '_online_' num2str(online) '.mat']);
            ...
            elseif strcmp(plot_type, 'propsig') 
                input = load(['output_varana/smoothing' '/' 'stats_varanalysis' ...
                    '_story_' num2str(s) '_nsets_' num2str(n_sets) ...
                    '_smoothtype_' smoothtype '_wsize_' num2str(wsize(w)) ...
                    '_dosplit_' num2str(0) '_splitby_' '' '_split_' num2str(0) ...
                    '_splittrun_' num2str(0) '_online_' num2str(online) '.mat']);
