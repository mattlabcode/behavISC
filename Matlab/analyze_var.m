...
sets = load([datadir '/' 'sets' '_story_' num2str(story(st)) '_nsets_' num2str(n_sets)...
    ...
    '_trunc_no' '_online_' num2str(online(o)) '.mat']);
sets = sets.sets;

ts = load(['timeseries' '/' 'ts' '_dosplit_' num2str(do_split) ...
    '_splitby_' split_by '_splittrun_' num2str(split_trun) ...
    '_online_'  num2str(online(o)) '.mat']);
ts   = ts.ts;

% savefile
savefile_fig = ['figures_ts' '/' 'ts_varanalysis' '_story_' num2str(story(st)) ...
    ...
    '_splittrun_' num2str(split_trun) '_online_' num2str(online(o)) '.svg'];

outdir = 'output_varana';
if length(wsize) > 1
    outdir = 'output_varana/smoothing';
end
savefile_stats = [outdir '/' 'stats_varanalysis' '_story_' num2str(story(st)) ...
    '_nsets_' num2str(n_sets) '_smoothtype_' smoothtype '_wsize_' num2str(wsize(w)) ...
    '_dosplit_' num2str(do_split) '_splitby_' split_by '_split_' num2str(split) ...
