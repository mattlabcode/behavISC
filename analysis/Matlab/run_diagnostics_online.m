...
% input 
[~, ~, raw] = xlsread('/Users/matthewbain/Documents/Science/Experiments/efflisisc/data/efflisisc_online.xlsx');
...
scram_col    = find(strcmp(raw(1, :), 'scramStory'));   % 'arctic' or 'swim'
...
browsresp_col = find(strcmp(raw(1, :), 'responses'));

% define logfile fields based on headers
log_hpfield    = 'hpass';
log_dtfields   = {'dtstim', 'dtresp', 'rt', 'dtcorr', 'ttime', 'tonset', 'story', 'scram', 'compresp', 'engresp', 'survq', 'survresp'}; 
log_compfields = {'compstim', 'compresp', 'compcorr'}; 
log_engfields  = {'engstim', 'engresp'}; 
log_survfields = {'survq', 'survresp', 'browsresp'}; 
...
for s = 1:length(subjs)
    ...
    surv_range        = find(strcmp(raw_curr(:, section_col), 'survey'));
    ...
    for b = 1:n_blocks    
        ...
        dt_range      = find(strcmp(raw_curr(:, section_col), [cond_ids{b} '-dual-task']));
        sect_ix(b).dt = [dt_range(1) dt_range(end)];

        % identify comprehension sections
        comp_range      = find(strcmp(raw_curr(:, section_col), [cond_ids{b} '-comprehension']));
        sect_ix(b).comp = [comp_range(1) comp_range(end)];
        
        % identify engagement sections
        eng_range       = find(strcmp(raw_curr(:, section_col), [cond_ids{b} '-engagement']));
        ...
        % scrambled story id
        if strcmp(log.scram{1}, 'arctic')
            dx.scram = 1;
        elseif strcmp(log.scram{1}, 'swim')
            dx.scram = 2;
        end
        
        %dx.scram = {log.scram{1}};
        ...
        dx.browser = {log.browsresp{strcmp(log.survq, 'browser')}};
        ...
        % reject subj -----------------------------------------------------
        % set comp_score rejection threshold based on whether current story scrambled or not
        if dx.story == dx.scram
            comp_thresh = 50;
        else
            comp_thresh = 70;
        end
        ...
        % reject
        if dx.comp_score < comp_thresh || dx.dtscore < .85 || dx.n_miss > 14 || contains(dx.browser, 'Safari')
            dx.rejected = 1;
            nrej.all = nrej.all + 1;
            if dx.hpcorr < 5 
                nrej.hp = nrej.hp + 1;
...
% array of subjects with at least one run (better to use for demo calcs)
onerun = any(~[dxav(1).rejected; dxav(2).rejected]);

n_subj = length(dxav(1).rejected);
n_subj_onerun = nnz(onerun);
n_blocksrej = nrej.all;
n_female = nnz(strcmp(dxav(1).gender(onerun), 'Female'));
age_min  = min(dxav(1).age(onerun));
age_max  = max(dxav(1).age(onerun));
age_mean = nanmean(dxav(1).age(onerun));

fprintf('n_subj: %d\n', n_subj);
fprintf('n_subj_onerun: %d\n', n_subj_onerun);
fprintf('n_blocksrej: %d\n', n_blocksrej);
fprintf('n_female: %d\n', n_female);
fprintf('age_min: %d\n', age_min);
fprintf('age_max: %d\n', age_max);
fprintf('age_mean: %d\n', age_mean);
...
% --- plot number of participants in each condition and timing version ---
condvers_dist = zeros(4, 6);

% loop through blocks and subject
for b = 1:length(dxav)
    for s = 1:length(dxav(1).story)
        
        if dxav(b).rejected(s) == 1
            continue
        end
        
        % add each block to 4•6 matrix based on condition/version
        if dxav(b).story(s) == 1 && dxav(b).scram(s) == 2
            condvers_dist(1, dxav(b).version(s)) = condvers_dist(1, dxav(b).version(s)) + 1;
        elseif dxav(b).story(s) == 1 && dxav(b).scram(s) == 1
            condvers_dist(2, dxav(b).version(s)) = condvers_dist(2, dxav(b).version(s)) + 1;
        elseif dxav(b).story(s) == 2 && dxav(b).scram(s) == 1
            condvers_dist(3, dxav(b).version(s)) = condvers_dist(3, dxav(b).version(s)) + 1;
        elseif dxav(b).story(s) == 2 && dxav(b).scram(s) == 2
            condvers_dist(4, dxav(b).version(s)) = condvers_dist(4, dxav(b).version(s)) + 1;    
        end
    end
end

% plot ---
xvalues = {'v1', 'v2', 'v3', 'v4', 'v5', 'v6'};
yvalues = {'Arctic intact', 'Arctic scrambled', 'Space intact', 'Space scrambled'};
h = heatmap(xvalues, yvalues, condvers_dist);

h.Title = 'Participants in each condition/timing version';
h.XLabel = 'version';
h.YLabel = 'condition';
...
print(gcf, ['Group_Processing' '/' 'participant_dist' '.svg'], '-dsvg', '-r300')    
